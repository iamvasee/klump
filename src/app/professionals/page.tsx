...
export default function ProfessionalsPage() {
  const breadcrumbs = [
    { label: 'Professionals', current: true }
  ];
  ...
  const professionals = people.filter(person => {
    return relationships.some(rel => 
      rel.person_id === person.id && 
      ['auditor', 'company_secretary'].includes(rel.role)
    );
  }).map(person => {
    const roles = Array.from(new Set(relationships
      .filter(rel => rel.person_id === person.id && ['auditor', 'company_secretary'].includes(rel.role))
      .map(rel => rel.role === 'auditor' ? 'Statutory Auditor' : 'Company Secretary')
    ));
    
    const entityCount = relationships.filter(rel => 
      rel.person_id === person.id && 
      ['auditor', 'company_secretary'].includes(rel.role)
    ).length;
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">Statutory Auditors and Company Secretaries</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProfessionals.map((pro) => (
            <div key={pro.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-all group">
              ...
              <div className="space-y-1 mb-6">
                <h3 className="text-lg font-bold text-gray-900 leading-tight">{pro.full_name}</h3>
                <div className="flex flex-wrap gap-2 pt-1">
                  {pro.professionalRoles.map(role => (
                    <span key={role} className="px-2 py-0.5 bg-blue-50 text-blue-700 text-[10px] font-bold uppercase tracking-wider rounded-md border border-blue-100">
                      {role}
                    </span>
                  ))}
                </div>
          ...
          {filteredProfessionals.length === 0 && (
            <div className="col-span-full py-12 text-center bg-white rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-500 font-medium">No professionals found matching your search.</p>
            </div>
          )}
        </div>
      </div>
