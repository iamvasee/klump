...
Branch: ${acc.branch || 'N/A'}`;
  ...
  const getDocumentIcon = (type: string) => {
    ...
    if (type.includes('itr') || type.includes('tds'))
  ...
  const staticDocuments =
    person.documents?.filter((doc) =>
      staticDocTypes.includes(doc.document_type)
    ) || [];

  // Recurring filings
  const recurringFilings =
    person.documents?.filter(
      (doc) => !staticDocTypes.includes(doc.document_type)
    ) || [];

  const filteredFilings = recurringFilings.filter((doc) => {
    const matchesSearch =
      doc.file_name.toLowerCase().includes(filingSearch.toLowerCase()) ||
      (doc.description || '')
        .toLowerCase()
        .includes(filingSearch.toLowerCase());
    const matchesFilter =
      filingFilter === 'all' ||
      getFilingCategory(doc.document_type) === filingFilter;
                  ...
                  <div className="flex-1 space-y-4">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span
                          className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${getStatusColor(person.completeness_score || 0)}`}
                        >
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-gray-50/50 rounded-2xl p-6 border border-gray-100">
                      <p className="text-gray-700 font-medium leading-relaxed">
                        {person.residential_address || 'Address not provided'}
                        <br />
